...
func (b *albcBuilder) withCredSecret(name string) *albcBuilder {
	if name != "" {
		b.credentials = &albo.SecretReference{
			Name: name,
		}
	}
	return b
...
// albcCredentialsRequest returns CredentialsRequest with IAM policies required by ALBC.
func albcCredentialsRequest(name types.NamespacedName, secretRef corev1.ObjectReference, saName string) (*cco.CredentialsRequest, error) {
